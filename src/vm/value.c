...
/*
 * error 传播：如果操作数已经是 error，直接透传（短路）。
 * 如果类型不支持该操作，返回 error value 而非 panic。
 */
...
value_t value_clone(vm_t *vm, value_t v) {
    ...
    if (v.type->vtable && v.type->vtable->clone) {
        ...
        if (v.type->size > 0 && v.data) {
            r.data = value_alloc_data_copy(vm->alloc, v.type, v.data);
        } else {
            r.data = NULL;
        }
    }
    ...
    /* 自动注册到 current_scope */
    if (r.type) {
        scope_track(vm, vm->current_scope, r);
    }
    return r;
}
...
value_t value_implicit_cast(vm_t *vm, value_t v, const type_t *target) {
    ...
    if (v.type->vtable && v.type->vtable->implicit_cast) {
        value_t result = v.type->vtable->implicit_cast(vm, v, target);
        if (value_is_error(vm, &result)) {
            return result;  /* error 已由 value_make_error 自动 track */
        }
        /* track cast 结果到当前 scope */
        scope_track(vm, vm->current_scope, result);
        return result;
    }
...
value_t value_explicit_cast(vm_t *vm, value_t v, const type_t *target) {
    ...
    if (v.type->vtable && v.type->vtable->explicit_cast) {
        value_t result = v.type->vtable->explicit_cast(vm, v, target);
        if (value_is_error(vm, &result)) {
            return result;
        }
        scope_track(vm, vm->current_scope, result);
