...
static size_t mem_search(const char *hay,
                         size_t hay_len,
                         const char *needle,
                         size_t needle_len,
                         size_t start) {
  if (needle_len == 0) return start;
  if (needle_len > hay_len || start > hay_len - needle_len) return STRING_NPOS;
...
string_t *string_from_string(allocator_t *allocator, const string_t *other) {
  if (!allocator || !other) return NULL;
  return string_from_bytes(
      allocator, other->data ? other->data : "", other->len);
}
...
void string_append_bytes(string_t *str, const char *data, size_t len) {
  if (!str || !data || len == 0) return;
  if (len > SIZE_MAX - str->len - 1) panic("string: capacity overflow");
  size_t need = str->len + len + 1;
  if (need > str->cap) string_grow(str, need);
...
void string_append_string(string_t *str, const string_t *other) {
  if (!str || !other) return;
  if (other->len > 0) string_append_bytes(str, other->data, other->len);
}
...
void string_shrink_to_fit(string_t *str) {
  if (!str || str->len + 1 == str->cap) return;
...
size_t string_find(const string_t *str, const char *needle, size_t start) {
  if (!str || !needle) return STRING_NPOS;
  size_t nlen = strlen(needle);
  if (nlen == 0) return start <= str->len ? start : STRING_NPOS;
  if (start > str->len) return STRING_NPOS;
...
size_t string_rfind(const string_t *str, const char *needle) {
  if (!str || !needle) return STRING_NPOS;
  size_t nlen = strlen(needle);
  if (nlen == 0) return str->len;
  if (nlen > str->len) return STRING_NPOS;
  for (size_t i = str->len - nlen;; i--) {
    if (memcmp(str->data + i, needle, nlen) == 0) return i;
    if (i == 0) break;
  }
  return STRING_NPOS;
}
...
string_concat(allocator_t *allocator, const string_t *a, const string_t *b) {
  if (!allocator || !a || !b) return NULL;
  if (a->len > SIZE_MAX - b->len - 1) panic("string_concat: length overflow");

  string_t *out = string_new(allocator);
  if (!out) return NULL;
  string_grow(out, a->len + b->len + 1);
  if (a->len > 0) memcpy(out->data, a->data, a->len);
  if (b->len > 0) memcpy(out->data + a->len, b->data, b->len);
  out->len = a->len + b->len;
  out->data[out->len] = '\0';
  return out;
}
...
string_t *string_replace(allocator_t *allocator,
  ...
  if (!allocator || !str || !needle) return NULL;
  if (needle[0] == '\0') return string_from_string(allocator, str);

  size_t nlen = strlen(needle);
  size_t rlen = replacement ? strlen(replacement) : 0;
  size_t pos = mem_search(str->data, str->len, needle, nlen, 0);
  if (pos == STRING_NPOS) return string_from_string(allocator, str);
  ...
  if (pos > 0) memcpy(out->data, str->data, pos);
  if (rlen > 0) memcpy(out->data + pos, replacement, rlen);
  memcpy(out->data + pos + rlen, str->data + pos + nlen, str->len - pos - nlen);
  out->len = new_len;
  out->data[new_len] = '\0';
  return out;
}
...
string_t *string_replace_all(allocator_t *allocator,
  ...
  for (;;) {
    ...
    if (i == STRING_NPOS) break;
    count++;
    i += nlen;
  }
  if (count == 0) return string_from_string(allocator, str);
  ...
  for (;;) {
    ...
    if (gap > 0) memcpy(out->data + dst_pos, str->data + src_pos, gap);
    dst_pos += gap;
    if (rlen > 0) memcpy(out->data + dst_pos, replacement, rlen);
    dst_pos += rlen;
    src_pos = i + nlen;
  }
  size_t tail = str->len - src_pos;
  if (tail > 0) memcpy(out->data + dst_pos, str->data + src_pos, tail);
  dst_pos += tail;
